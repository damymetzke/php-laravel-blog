...
# Test if variables are present that will be used later in the script:
...
if [[ ! -f .env ]]; then
  ...
  sed -i "s/MYSQL_ROOT_PASSWORD_FOR_DOCKER=.*/MYSQL_ROOT_PASSWORD_FOR_DOCKER=${docker_database_password}/" .env
